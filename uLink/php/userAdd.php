...
include("database_connection.php");
    ...
    if(isset($_POST["insert"]))
    {
        ...
        /*$request = $connect->prepare("INSERT INTO possess (id_user, id_permissions) VALUES (:id_user, '1')");
    ...
    elseif (!empty($_GET['action']) and $_GET["action"] == "update"){
        ...
        $request3 = $connect->prepare("SELECT * FROM roles NATURAL JOIN be NATURAL JOIN user WHERE id_user=:id_user");
        ...
        if ($isDelegate)
        {
            $request2 = $connect->prepare("SELECT * FROM permissions NATURAL JOIN possess NATURAL JOIN user WHERE id_user=:id_user");
